...
const tasksListScreen = {
  // Shared header renderer. includeArea toggles whether the Area column is shown.
  tableHeader(includeArea = true) {
    return `<tr>` +
      (includeArea ? `<th onclick="tasksListScreen.sortTable('area')">Area ↕</th>` : ``) +
  ...
  // Shared single-row renderer. includeArea must match header.
  rowHtml(task, includeArea = true) {
    ...
    return `<tr>` +
      (includeArea ? `<td>${escapeHtml(getTaskAreaLabel(task))}</td>` : ``) +
  ...
  render() {
    ...
    let taskRows = displayTasks.length === 0
      ? `<tr><td colspan="12" style="text-align:center;padding:30px">No tasks found</td></tr>`
      : displayTasks.map(task => this.rowHtml(task, true)).join("");

    document.getElementById("taskTable").innerHTML = `${this.tableHeader(true)}${taskRows}`;
  },

  // Backwards-compatible single-row renderer alias
  renderRow(task) { return this.rowHtml(task, true); },
